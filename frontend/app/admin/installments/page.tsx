...
export default function InstallmentsPage() {
  ...
  const handleAddDiscount = (student: StudentWithInstallments) => {
    setSelectedStudent(student);
    const totalOutstanding = parseFloat(String(student.outstanding?.totalOutstanding || '0'));
    setSelectedStudentTotalOutstanding(totalOutstanding);
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-lg ${
                    discountForm.amount && parseFloat(discountForm.amount) > selectedStudentTotalOutstanding
                ...
                {discountForm.amount && parseFloat(discountForm.amount) > selectedStudentTotalOutstanding && (
                  <p className="mt-1 text-sm text-red-600">
                    {(t.installments?.discountAmountExceedsError || 'Discount amount cannot exceed total outstanding balance ({amount})').replace('{amount}', formatCurrency(selectedStudentTotalOutstanding))}
                  </p>
                )}
              </div>
              <div>
              ...
              <div className="flex gap-3">
                <button
                  onClick={async () => {
                    ...
                    if (isNaN(discountAmount) || !discountForm.amount || discountAmount <= 0) {
                      setError(t.installments?.invalidDiscountAmount || 'Please enter a valid discount amount');
                      return;
                    }
                    if (discountAmount > selectedStudentTotalOutstanding) {
                      setError((t.installments?.discountAmountExceedsError || 'Discount amount cannot exceed total outstanding balance ({amount})').replace('{amount}', formatCurrency(selectedStudentTotalOutstanding)));
                      return;
                    }
                    try {
                  ...
                  disabled={!discountForm.amount || isNaN(parseFloat(discountForm.amount)) || parseFloat(discountForm.amount) <= 0 || parseFloat(discountForm.amount) > selectedStudentTotalOutstanding}
